...
public abstract class PackagerBlockEntityMixin {
    ...
    @ModifyExpressionValue(method = "getLinkPos", at = @At(value = "INVOKE", target = "Lcom/tterrag/registrate/util/entry/BlockEntry;has(Lnet/minecraft/world/level/block/state/BlockState;)Z"))
    private boolean createVibrantVaults$getLinkPosHasStockLink(boolean original, @Local BlockState adjacentState) {
        return original || ModBlockTags.VIBRANT_STOCK_LINKS.matches(adjacentState);
    }

    @ModifyExpressionValue(method = "flashLink", at = @At(value = "INVOKE", target = "Lcom/tterrag/registrate/util/entry/BlockEntry;has(Lnet/minecraft/world/level/block/state/BlockState;)Z"))
